...
class Trainer(object):
    ...
    def save_state_dict(self, filename, trainable=True):
        state_dict = edict()
        ...
        if trainable:
            state_dict.solver = self.temps.base_solver.state_dict()
            state_dict.scheduler = self.temps.scheduler.state_dict()
    ...
    def load_state_dict(self, filename):
        ...
        for name, model in saved_dict.models.items():
            if isinstance(self.models.get(name), torch.nn.Module):
        ...
        self.temps.base_solver_dict=saved_dict.solver
        self.temps.scheduler_dict=saved_dict.scheduler
        return self
